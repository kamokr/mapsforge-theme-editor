...
class ThemeTreeTransferHandler extends TransferHandler {
    ...
    public boolean canImport(TransferHandler.TransferSupport support) {
        ...
        // Check for self-drop or dropping on descendants
        ...
        // Get the target node (where we're dropping)
        DefaultMutableTreeNode targetNode =
                (DefaultMutableTreeNode) dl.getPath().getLastPathComponent();
        Object targetUserObject = targetNode.getUserObject();

        // Get the source nodes (what we're dragging)
        try {
            // The data is likely DefaultMutableTreeNode[] not TreePath[]
            DefaultMutableTreeNode[] draggedNodes = (DefaultMutableTreeNode[]) support.getTransferable().getTransferData(nodesFlavor);

            for (DefaultMutableTreeNode sourceNode : draggedNodes) {
                Object sourceUserObject = getActualUserObject(sourceNode);

                // Check drop restrictions based on types
                if (!isDropAllowed(sourceUserObject, targetUserObject)) {
                    return false;
                }
            }
        } catch (Exception e) {
    ...
    private Object getActualUserObject(DefaultMutableTreeNode node) {
        ...
        // If the user object is another DefaultMutableTreeNode, drill down
        while (userObject instanceof DefaultMutableTreeNode) {
    ...
    protected void exportDone(JComponent source, Transferable data, int action) {
        ...
        // Notify drop success listener
        if (dropSuccessListener != null) {
            dropSuccessListener.onDropSuccess();
        }
    }
    ...
    // Drop success listener
    private OnDropSuccessListener dropSuccessListener;
    public void setOnDropSuccessListener(OnDropSuccessListener dropSuccessListener) {
        this.dropSuccessListener = dropSuccessListener;
    }

    public interface OnDropSuccessListener {
        void onDropSuccess();
    }
}
